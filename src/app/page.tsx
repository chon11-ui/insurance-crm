...
export default async function Home({
  ...
  if (adminError) {
    return (
      <div className="p-8 text-red-600 max-w-4xl mx-auto mt-20 bg-white rounded shadow text-center">
        <h1 className="text-2xl font-bold mb-4">데이터베이스 연결 오류 (환경변수 오류)</h1>
        <p>Netlify에 등록된 Firebase 환경변수에 문제가 있습니다. 관리자에게 문의하세요.</p>
        <pre className="mt-4 bg-red-50 p-4 rounded text-sm overflow-auto text-left">{adminError}</pre>
        <form action={async () => {
          'use server'
          ...
          <button className="mt-6 bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg">다시 로그인</button>
        </form>
      </div>
    )
  }
  ...
  try {
    ...
    return (
      <div className="p-8 text-red-600 max-w-4xl mx-auto mt-20 bg-white rounded shadow text-center">
        <h1 className="text-2xl font-bold mb-4">화면 로딩 실패 (데이터베이스 에러)</h1>
        <p>고객 목록을 가져오는데 실패했습니다.</p>
        <pre className="mt-4 bg-red-50 p-4 rounded text-sm overflow-auto text-left">{error.message || String(error)}</pre>
        <form action={async () => {
          'use server'
          const { removeSession } = await import('./actions/auth')
          await removeSession()
